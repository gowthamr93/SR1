...
[assembly: Xamarin.Forms.Dependency(typeof(ServiceRequest.Droid.DependencyClasses.FileSystem))]
namespace ServiceRequest.Droid.DependencyClasses
{
    public class FileSystem : IFileSystem
    {
        ...
        {
            var rootDir = new Java.IO.File(Android.OS.Environment.ExternalStorageDirectory + @"/IdoxSRi");
            if (!rootDir.Exists())
                rootDir.Mkdirs();
            return rootDir.Path;
        }
        ...
        public async Task<FileSystemArgs> DeleteAsync(params string[] path)
        {
            ...
            FileSystemArgs args;
            //
            var filePath = Path.Combine(await Paths(path));
            try
            {
